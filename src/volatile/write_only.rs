...
impl<Addr, Volatile> VolatileWriteOnly<Addr, Volatile> {
    pub(crate) const fn new(config: Config<Addr>) -> VolatileWriteOnly<Addr, Volatile> {
        Self {
            config,
            _marker: PhantomData,
        }
...
fn mask(
    ...
    bits: usize,
    offset: usize,
) -> core::result::Result<usize, WriteErr> {
...
impl_write_only!(u8, u16, u32, u64, usize);
...
mod tests {
    ...
    fn it_write_volatile() {
        ...
        let w = Builder::new(buff.as_ptr() as u64).build_write_only_type_as::<u32>();
        w.write_volatile(0b100).unwrap();

        let r = Builder::new(buff.as_ptr() as u64).build_readonly_type_as::<u32>();
        assert_eq!(r.read_volatile(), 0b100);
    }

    #[test]
    fn it_write_volatile_with_offset() {
        let buff: [u64; 3] = [0b0000_1001; 3];
        let v = Builder::new(buff.as_ptr() as u64)
            .offset(4)
            .build_type_as::<u32>();

        v.write_volatile(0b100).unwrap();
    ...
    fn it_write_max_value() {
        let buff: [u64; 2] = [1; 2];
        let v = Builder::new(buff.as_ptr() as u64).build_type_as::<u64>();

        v.write_volatile(u64::MAX).unwrap();
        assert_eq!(v.read_volatile(), u64::MAX);
    }
